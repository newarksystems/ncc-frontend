...
const agents = [
    { agent: "Doris Glimp", total_calls: "72", ext: "1002", selected: false },
    { agent: "Britta Shop", total_calls: "79", ext: "1003", selected: false },
    { agent: "Mark Simon", total_calls: "88", ext: "1004", selected: false },
    { agent: "Susan Yavolsky", total_calls: "90", ext: "1005", selected: false },
    { agent: "Mary Homer", total_calls: "97", ext: "1006", selected: false }
];
...
export default function AgentOverviewPerformanceMetrics() {
    ...
    const [activeAgent, setActiveAgent] = useState("1002");

    const handleAgentClick = (ext) => {
        setActiveAgent(ext);
    };

    const currentAgentData = agentData[activeAgent] || agentData["1002"];
                        ...
                        <table className="w-full border-collapse">
                            ...
                            <tbody>
                                {agents.map((row, i) => (
                                    <tr
                                        key={i}
                                        onClick={() => handleAgentClick(row.ext)}
                                        className={`cursor-pointer ${activeAgent === row.ext ? 'bg-slate-700' : ''} hover:bg-slate-600`}
                                    >
